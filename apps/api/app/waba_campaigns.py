...
def send_campaign(campaign_id: str, user: User = Depends(require_user), db: Session = Depends(get_db)):
    """Sends synchronously, recipient by recipient -- acceptable for the realistic per-entity
    contact-list sizes this targets; a genuinely large-scale broadcast product would queue this,
    flagged as a real scaling limit worth revisiting if campaign sizes grow into the thousands."""
    entity = _resolve_entity(db, user)
    campaign = db.get(Campaign, campaign_id)
    if not campaign or campaign.entity_id != entity.id:
        raise HTTPException(status_code=404, detail="Campaign not found")
    ...
    if not template or template.get("status") != "APPROVED":
        raise HTTPException(status_code=422, detail="This template isn't APPROVED for sending yet")
    ...
    contacts = segment_matching_contacts(db, entity.id, segment) if segment else []
    ...
    for contact in contacts:
        ...
        if contact.opted_out or not contact.wa_id:
            ...
            continue
        try:
            send_whatsapp_template(db, entity.id, contact.wa_id, campaign.template_name, campaign.template_language, campaign.body_params, preview_body, sent_by_user_id=user.id)
